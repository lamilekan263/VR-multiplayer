...
public class NetworkXRRig : NetworkBehaviour
{
    ...
    public override void Spawned()
    {
        ...
        {
            localXRRig = FindFirstObjectByType<LocalXRRig>();
            if (localXRRig == null) Debug.LogError("Missing LocalXRRig in thescene");
        }
    }
public override void FixedUpdateNetwork()
    {
        ...
        if (GetInput<RigInput>(out var input))
        {
            ...
            rightHandAnimator.SetFloat("Trigger_Value",
            input.rightTriggerValue);
        }
    }
public override void Render()
    ...
    {
        base.Render();
        // Local simulation of Networked Rig using inputs captured by LocalXR Rig
        if (IsbelongToLocalPlayer)
        {
            if (localXRRig != null)
            {
                ...
                localXRRig.leftHandTransform.position;
                leftHand.transform.rotation =
                localXRRig.leftHandTransform.rotation;
                rightHand.transform.position =
                localXRRig.rightHandTransform.position;
                rightHand.transform.rotation =
                localXRRig.rightHandTransform.rotation;
